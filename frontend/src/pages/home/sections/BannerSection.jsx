...
function BannerSection() {
  ...
  return (
    <div className="relative mt-80 xs:mt-6 sm:mt-118 md:mt-136 lg:mt-210 w-full">
      <div className="bg-blue-400 min-h-[200px] xs:min-h-[220px] sm:min-h-[250px] md:min-h-[280px] lg:min-h-[300px] w-full">
        <div className="flex flex-col items-center p-6 sm:p-8">
            ...
            {bannerImages.map((item, index) => (
              <div key={index} className="mt-3">
                <img
                  src={item.image}
                  alt={`Banner ${index + 1}`}
                  className="h-[60px] xs:h-[50px] sm:h-[80px] md:h-[100px] lg:h-[120px] w-auto object-contain"
                />
              </div>
            ))}
          </div>
          ...
          {/* Button */}
          <button className="mt-5 xs:mt-6 sm:mt-8 md:mt-10 lg:mt-12 px-6 py-2 text-white underline cursor-pointer font-semibold hover:text-gray-900 transition">
            Shop Now
          </button>
        </div>
      </div>
